#Stock Data Visualization APP code 
...
# Function to load stock data for two tickers within a specified time frame
...
# Function triggered on button click to load and plot stock data
def on_button_click(ticker1, ticker2, start, end, indicators):
    ...
    # Clear current document and update with new plots
    curdoc().clear()
    curdoc().add_root(layout)
    curdoc().add_root(row(p1, p2))

# User input elements
stock1_text = TextInput(title="Stock1")
stock2_text = TextInput(title="Stock2")
...
# Define button callback to trigger data loading
...
# Clear current document and set initial layout
curdoc().clear()
curdoc().add_root(layout)
